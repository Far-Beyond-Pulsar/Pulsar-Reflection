//! Fixed-size matrix arrays used by renderer component reflection.
...
#[pulsar_type(
    serialize_json_with = serialize_mat3x4_json,
    deserialize_json_with = deserialize_mat3x4_json
)]
type RegisteredMat3x4 = [[f32; 4]; 3];
...
type RegisteredF32x12 = [f32; 12];#[pulsar_type(serialize_json_with = serialize_f32x2_json, deserialize_json_with = deserialize_f32x2_json)]
...
fn serialize_mat3x4_json(value: &[[f32; 4]; 3]) -> crate::ReflectResult<serde_json::Value> {
    Ok(serde_json::json!(value))
}
...
fn deserialize_mat3x4_json(value: serde_json::Value) -> crate::ReflectResult<[[f32; 4]; 3]> {
    serde_json::from_value(value).map_err(|error| crate::ReflectError::TypeMismatch {
        expected: "[[f32; 4]; 3]",
        found: error.to_string(),
    })
